...
public class DescriptionParserTests
{
    ...
    private readonly string _coloredTextScaling1Corrected = "<c val=\"#TooltipNumbers\">100</c> (+4% per level)<n/> damage per second<n/>";
    private readonly string _coloredTextScaling2 = "<c val=\"#TooltipNumbers\">100~~0.04~~</c> damage per second ~~0.05~~";
    private readonly string _coloredTextScaling2Corrected = "<c val=\"#TooltipNumbers\">100</c> (+4% per level) damage per second  (+5% per level)";

    // text with error tag
    private readonly string _errorText1 = "<c val=\"#TooltipNumbers\">100##ERROR##~~0.04~~</c> damage per second<n/>";
    private readonly string _errorText1Corrected = "<c val=\"#TooltipNumbers\">100</c> (+4% per level) damage per second<n/>";
    ...
    {
        Assert.AreEqual(_noTagsDescription, DescriptionParser.Validate(_noTagsDescription)); // no changes
        Assert.AreEqual(_normalTagsDescription1, DescriptionParser.Validate(_normalTagsDescription1)); // no changes
        Assert.AreEqual(_normalTagsDescription2, DescriptionParser.Validate(_normalTagsDescription2)); // no changes
        Assert.AreEqual(_noTagsDescription, DescriptionParser.Validate(_extraTagDescription1));
        Assert.AreEqual(_noTagsDescription, DescriptionParser.Validate(_extraTagDescription2));
        Assert.AreEqual(_noTagsDescription, DescriptionParser.Validate(_extraTagDescription3));
        Assert.AreEqual(_noTagsDescription, DescriptionParser.Validate(_extraTagDescription4));
        Assert.AreEqual(_newLineTagDescription1, DescriptionParser.Validate(_newLineTagDescription1)); // no changes
        Assert.AreEqual(_newLineTagDescription2, DescriptionParser.Validate(_newLineTagDescription2)); // no changes
        Assert.AreEqual(_selfCloseTagDescription1, DescriptionParser.Validate(_selfCloseTagDescription1)); // no changes
        Assert.AreEqual(_selfCloseTagDescription2, DescriptionParser.Validate(_selfCloseTagDescription2)); // no changes
        Assert.AreEqual(_selfCloseTagDescription3, DescriptionParser.Validate(_selfCloseTagDescription3)); // no changes
        Assert.AreEqual(_selfCloseTagDescription4, DescriptionParser.Validate(_selfCloseTagDescription4)); // no changes
        Assert.AreEqual(_normalTagsDescription2, DescriptionParser.Validate(_duplicateTagsDescription1));
        Assert.AreEqual(_normalTagsDescription2, DescriptionParser.Validate(_duplicateTagsDescription2));
        Assert.AreEqual(_spaceTagDescription1, DescriptionParser.Validate(_spaceTagDescription1)); // no changes
        Assert.AreEqual(_spaceTagDescription2, DescriptionParser.Validate(_spaceTagDescription2)); // no changes
    }

    [TestMethod]
    public void ValidateBrokenTagsTest()
    {
        Assert.AreEqual(_incompleteTagDescription1Corrected, DescriptionParser.Validate(_incompleteTagDescription1));
        Assert.AreEqual(_incompleteTagDescription2Corrected, DescriptionParser.Validate(_incompleteTagDescription2));
        Assert.AreEqual(_incompleteTagDescription3Corrected, DescriptionParser.Validate(_incompleteTagDescription3));
        Assert.AreEqual(_incompleteTagDescription4Corrected, DescriptionParser.Validate(_incompleteTagDescription4));
        Assert.AreEqual(_incompleteTagDescription5Corrected, DescriptionParser.Validate(_incompleteTagDescription5));
    }

    [TestMethod]
    public void ValidateConvertedNewlineTagsTest()
    {
        Assert.AreEqual(_convertNewLineTagDescription1Corrected, DescriptionParser.Validate(_convertNewLineTagDescription1));
        Assert.AreEqual(_convertNewLineTagDescription2Corrected, DescriptionParser.Validate(_convertNewLineTagDescription2));
        Assert.AreEqual(_convertNewLineTagDescription3Corrected, DescriptionParser.Validate(_convertNewLineTagDescription3));
    }

    [TestMethod]
    public void ValidateCaseTagsTest()
    {
        Assert.AreEqual(_upperCaseTagDescription1Corrected, DescriptionParser.Validate(_upperCaseTagDescription1));
    }

    [TestMethod]
    public void ValidateExtraSpaceInTagsTest()
    {
        Assert.AreEqual(_normalTagsDescription2, DescriptionParser.Validate(_extraSpacesTagDescription1));
        Assert.AreEqual(_normalTagsDescription2, DescriptionParser.Validate(_extraSpacesTagDescription2));
    }

    [TestMethod]
    public void ValidateEmptyTagsTest()
    {
        Assert.AreEqual(_emptyTagsDescription1Corrected, DescriptionParser.Validate(_emptyTagsDescription1));
        Assert.AreEqual(_emptyTagsDescription2Corrected, DescriptionParser.Validate(_emptyTagsDescription2));
        Assert.AreEqual(_emptyTagsDescription3Corrected, DescriptionParser.Validate(_emptyTagsDescription3));
    }

    [TestMethod]
    public void ValidateNestedTagsTest()
    {
        Assert.AreEqual(_nestedTagDescription1Corrected, DescriptionParser.Validate(_nestedTagDescription1));
        Assert.AreEqual(_nestedTagDescription2Corrected, DescriptionParser.Validate(_nestedTagDescription2));
        Assert.AreEqual(_nestedTagDescription3Corrected, DescriptionParser.Validate(_nestedTagDescription3));
        Assert.AreEqual(_nestedTagDescription4Corrected, DescriptionParser.Validate(_nestedTagDescription4));
    }

    [TestMethod]
    public void ValidateNestedNewLineTagsTest()
    {
        Assert.AreEqual(_nestedNewLineTagDescription1Corrected, DescriptionParser.Validate(_nestedNewLineTagDescription1));
        Assert.AreEqual(_nestedNewLineTagDescription2Corrected, DescriptionParser.Validate(_nestedNewLineTagDescription2));
    }

    [TestMethod]
    public void ValidateRealDescriptionTest()
    {
        Assert.AreEqual(_diabloBlackSoulstoneCorrected, DescriptionParser.Validate(_diabloBlackSoulstone));
        Assert.AreEqual(_dvaMechSelfDestructCorrected, DescriptionParser.Validate(_dvaMechSelfDestruct));
        Assert.AreEqual(_valeeraCheapShotCorrected, DescriptionParser.Validate(_valeeraCheapShot));
        Assert.AreEqual(_crusaderPunishSame, DescriptionParser.Validate(_crusaderPunish));
        Assert.AreEqual(_meiSnowBlindDeDECorrected, DescriptionParser.Validate(_meiSnowBlindDeDE));
    }

    [TestMethod]
    public void ValidatePlainTextTest()
    {
        Assert.AreEqual(_plainText1, DescriptionParser.GetPlainText(_nestedTagDescription1, false, false));
        Assert.AreEqual(_plainText2, DescriptionParser.GetPlainText(_nestedNewLineTagDescription2Corrected, false, false));
        Assert.AreEqual(_plainText3, DescriptionParser.GetPlainText(_valeeraCheapShotCorrected, false, false));
        Assert.AreEqual(_plainText4Corrected, DescriptionParser.GetPlainText(_plainText4, false, false));
        Assert.AreEqual(_plainText5Corrected, DescriptionParser.GetPlainText(_plainText5, false, false));
        Assert.AreEqual(_plainText6Corrected, DescriptionParser.GetPlainText(_plainText6, false, false));
        Assert.AreEqual(_plainTextMeiSnowBlindDeDECorrected, DescriptionParser.GetPlainText(_meiSnowBlindDeDECorrected, false, false, StormLocale.DEDE));
    }

    [TestMethod]
    public void ValidatePlainTextNewlineTest()
    {
        Assert.AreEqual(_plainTextNewline1, DescriptionParser.GetPlainText(_nestedNewLineTagDescription1Corrected, true, false));
        Assert.AreEqual(_plainTextNewline2, DescriptionParser.GetPlainText(_valeeraCheapShotCorrected, true, false));
        Assert.AreEqual(_plainTextNewline3Corrected, DescriptionParser.GetPlainText(_plainTextNewline3, true, false));
    }

    [TestMethod]
    public void ValidatePlainTextScalingTest()
    {
        Assert.AreEqual(_plainTextScaling1Corrected, DescriptionParser.GetPlainText(_plainTextScaling1, false, true));
        Assert.AreEqual(_plainTextScaling2Corrected, DescriptionParser.GetPlainText(_plainTextScaling2, false, true));
        Assert.AreEqual(_plainTextScalingMeiSnowBlindDeDECorrected, DescriptionParser.GetPlainText(_meiSnowBlindDeDECorrected, false, true, StormLocale.DEDE));
    }

    [TestMethod]
    public void ValidatePlainTextScalingNewlineTest()
    {
        Assert.AreEqual(_plainTextScalingNewline1Corrected, DescriptionParser.GetPlainText(_plainTextScalingNewline1, true, true));
        Assert.AreEqual(_plainTextScalingNewline2Corrected, DescriptionParser.GetPlainText(_plainTextScalingNewline2, true, true));
    }

    [TestMethod]
    public void ValidateColoredTextTest()
    {
        Assert.AreEqual(_coloredText1Corrected, DescriptionParser.GetColoredText(_coloredText1, false));
        Assert.AreEqual(_coloredText2Corrected, DescriptionParser.GetColoredText(_coloredText2, false));
    }

    [TestMethod]
    public void ValidateColoredTextScalingTest()
    {
        Assert.AreEqual(_coloredTextScaling1Corrected, DescriptionParser.GetColoredText(_coloredTextScaling1, true));
        Assert.AreEqual(_coloredTextScaling2Corrected, DescriptionParser.GetColoredText(_coloredTextScaling2, true));
    }

    [TestMethod]
    public void ValidateErrorTextTest()
    {
        Assert.AreEqual(_errorText1Corrected, DescriptionParser.GetColoredText(_errorText1, true));
        Assert.AreEqual(_errorText2Corrected, DescriptionParser.GetPlainText(_errorText2, false, false));
        Assert.AreEqual(_errorText3Corrected, DescriptionParser.GetPlainText(_errorText3, true, false));
    }

    [TestMethod]
    public void ValidateSpaceTagsTest()
    {
        Assert.AreEqual(_spaceTagDescription1, DescriptionParser.GetColoredText(_spaceTagDescription1, true));
        Assert.AreEqual(_spaceTagDescription2, DescriptionParser.GetColoredText(_spaceTagDescription2, true));
        Assert.AreEqual(_spaceTagDescription3, DescriptionParser.GetColoredText(_spaceTagDescription3, true));
        Assert.AreEqual(_spaceTagNormalDescription1, DescriptionParser.GetPlainText(_spaceTagDescription1, false, true));
        Assert.AreEqual(_spaceTagNormalDescription2, DescriptionParser.GetPlainText(_spaceTagDescription2, true, true));
        Assert.AreEqual(_spaceTagNormalDescription3, DescriptionParser.GetPlainText(_spaceTagDescription3, true, true));
    }
}
